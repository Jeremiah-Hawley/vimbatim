...
impl AppState {
    ...
    fn push_empty_tab(&mut self) -> TabId {
    ...
    fn continue_vim_keybind_sequence(
        &mut self,
        key: &str,
        shift: bool,
        key_char: Option<&str>,
    ...
    fn try_handle_vim_register_prefix(
        &mut self,
        key: &str,
        shift: bool,
        key_char: Option<&str>,
...
fn linewise_bounds_for_operator(
    operator: char,
    start: usize,
    end: usize,
    content: &str,
...
mod tests {
    ...
    fn open_file_new_tab_requests_editor_focus() {
        let dir =
            std::env::temp_dir().join(format!("vimbatim_focus_test_{}_new", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn moving_a_file_also_moves_it_on_the_reopen_stack() {
        ...
        crate::docx_parser::create_new_docx(&default_paragraphs(), &src, Default::default())
            .unwrap();
    ...
    fn a_new_document_is_a_complete_package_with_no_dangling_references() {
        ...
        assert!(
            styles[styles.find("w:styleId=\"Heading2\"").unwrap()..]
                .contains("<w:sz w:val=\"38\"/>"),
    ...
    fn through_a_real_docx(paragraphs: Vec<Paragraph>, label: &str) -> AppState {
        let dir =
            std::env::temp_dir().join(format!("vimbatim_menu_{}_{}", std::process::id(), label));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("doc.docx");
    ...
    fn test_selection_font_size_reports_zero_for_an_unstyled_run() {
        let mut state =
            make_state_with_paragraphs(vec![sized_para(vec![sized_run("plain", 0)])], 0);
        state.workspace.tabs[0].selection = Some((0, 5));
    ...
    fn test_selection_font_size_at_start_of_text_reports_the_first_run() {
        let mut state =
            make_state_with_paragraphs(vec![sized_para(vec![sized_run("hello", 48)])], 0);
        state.workspace.tabs[0].selection = None;
        assert_eq!(state.selection_font_size_half_points(), Some(48));
    }
    ...
    fn test_selection_font_size_treats_a_collapsed_selection_as_a_caret() {
        let mut state =
            make_state_with_paragraphs(vec![sized_para(vec![sized_run("hello", 48)])], 5);
    ...
    fn test_set_font_size_applies_to_the_selection() {
        let mut state =
            make_state_with_paragraphs(vec![sized_para(vec![sized_run("hello", 0)])], 0);
        state.workspace.tabs[0].selection = Some((0, 5));
    ...
    fn make_state_with_recovery(tag: &str) -> (AppState, std::path::PathBuf) {
        let dir =
            std::env::temp_dir().join(format!("vimbatim-state-rec-{}-{tag}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
